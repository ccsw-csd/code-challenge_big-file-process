...
public class BigFileProcessCodeChallenge {
    ...
    private void extractStatistics(Map<String, Map<String, Integer>> map) {

        Optional<Map.Entry<String, Integer>> womenMax = map.get("Mujer").entrySet().stream().max(Map.Entry.comparingByValue());
        ...
        Optional<Map.Entry<String, Integer>> womenMin = map.get("Mujer").entrySet().stream().min(Map.Entry.comparingByValue());
        ...
        Optional<Map.Entry<String, Integer>> menMax = map.get("Hombre").entrySet().stream().max(Map.Entry.comparingByValue());
        ...
        Optional<Map.Entry<String, Integer>> menMin = map.get("Hombre").entrySet().stream().min(Map.Entry.comparingByValue());
    ...
    private  Map<String, Map<String, Integer>> readDataFile(String filename) throws IOException {

        try (Stream<String> lines = java.nio.file.Files.lines(Paths.get(filename))) {
            return lines.parallel().map(line -> line.split(";")).collect(Collectors.groupingBy(line -> line[1],
                    Collectors.groupingBy(line -> line[0] + "-" + line[2], Collectors.summingInt(e -> Integer.parseInt(e[5])))));
        }
    }
